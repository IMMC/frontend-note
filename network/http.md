...
![sameSite 示意图](https://user-gold-cdn.xitu.io/2020/3/19/170 f2fdc0467a90d?w=701&h=432&f=png&s=70893)
...
签名。一般由受信任的第三方 CA 机构签发。处于性能考虑，一般身份验证过程中是使用非对称加密，身份认证后，通过非对称加密传输对称加密的密钥，后续通讯就可以使用对称加密进行了，下面是较为详细的过程：
    ...
    > 4. 校验通过后，浏览器生成一个对称加密的密钥，使用证书中的钥进行加密发送给服务端，服务端使用私钥解密，之后的通讯加密就可以使用这个密钥进行通讯了。
